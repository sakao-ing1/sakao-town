...
public class SeuilCalcul extends JPanel {
	...
	public static final int NB_VEHICLES=4000;
	public static final int NB_TRAM=2000;
	public static final int NB_BIKE=1000;
	public static final int NB_Ped=3000;
	...
	public SeuilCalcul(AppStructureHandler appStructure) throws IOException, JSONException {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String s = al.get(0) + "\n";				
					SmartCity req1 = new ObjectMapper().readValue(s, SmartCity.class);
					double calcul1=calculS.estimatedThreshold(NB_VEHICLES , req1.getHeightkm(), NB_BIKE, NB_TRAM, NB_Ped);
		...
		btnReleod.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					{
						JOptionPane.showMessageDialog(null, "The inputs are incorrects, all the inputs must be greater then 0");
					}
					else {
						String s = al.get(0) + "\n";						
						SmartCity req1 = new ObjectMapper().readValue(s, SmartCity.class);
						double calcul1=calculS.estimatedThreshold((Integer) spinner.getValue(), 
								req1.getHeightkm(), 
								Integer.parseInt(textField_3.getText()), 
								Integer.parseInt(nbUsesTram90.getText()), 
								Integer.parseInt(textField.getText()));
						double sumFields=Integer.parseInt(textField_3.getText())+Integer.parseInt(nbUsesTram90.getText())+Integer.parseInt(textField.getText())+(Integer) spinner.getValue();
						if(sumFields>NB_POPULATION){
							JOptionPane.showMessageDialog(null, "The sums of the users of transport must be less than the number of population");
						}
						else {
		...
		btnConclusion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if ( param1 <param2) {
					JOptionPane.showMessageDialog(null, "the proposed travel policy is better than the current travel policy");
				}		
				else {
				JOptionPane.showMessageDialog(null, "the current travel policy is better than the proposed travel policy");
